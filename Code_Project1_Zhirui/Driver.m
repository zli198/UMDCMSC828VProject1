...
for i=5:6
    ...
    elseif i==2
        w = ones(dim^2+dim+1,1);
        [w,f,gnorm] = SG_Nesterov(fun,gfun,label,w,bsz,kmax,tol);
        W = reshape(w(1:dim^2),dim,dim);
        v = w(dim^2+1:dim^2+dim);
        b = w(end);
        % plot the objective function
    ...
    elseif i==3
        w = ones(dim^2+dim+1,1);
        [w,f,gnorm] = SG_Adam(fun,gfun,label,w,bsz,kmax,tol);
        W = reshape(w(1:dim^2),dim,dim);
        v = w(dim^2+1:dim^2+dim);
        b = w(end);
        % plot the objective function
    ...
    elseif i==4
        w = ones(dim^2+dim+1,1);
        [w,f,gnorm] = S_LBFGS(fun,gfun,label,w,bsz,kmax,tol);
        W = reshape(w(1:dim^2),dim,dim);
        v = w(dim^2+1:dim^2+dim);
        b = w(end);
        % plot the objective function
