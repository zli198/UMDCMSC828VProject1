...
kmax = 2e1*frac; % the max number of iterations
tol = 1e-6;
% call the optimizers
 [w,f,gnorm] = SG(fun,gfun,label,w,bsz,kmax,tol);
% [w,f,gnorm] = SNesterov(fun,gfun,label,w,bsz,kmax,tol);
% [w,f,gnorm] = SAdam(fun,gfun,label,w,bsz,kmax,tol);
% [w,f,gnorm] = SLBFGS(fun,gfun,label,w,kmax,tol);
% [w,f,gnorm] = GaussNewton(Xtrain,label,w,kmax,tol);
% [w,f,gnorm] = LevenbergMarquardt(Xtrain,label,w,kmax,tol);



wmat = reshape(w(1:dim^2),dim,dim);
...
test = testlabel.*(diag(Xtest*wmat*Xtest') + Xtest*v + b);
hits = find(test > 0);
misses = find(test < 0);
nhits = length(hits);
